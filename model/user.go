...
func (d userDao) RetrieveRecords(ctx *gin.Context, options ...func(db *gorm.DB) *gorm.DB) ([]User, error) {
	...
	if result.Error != nil {
		return records, errors.Wrapf(base.ErrorDBSelect, "retrieve records fail, err: %s", result.Error.Error())
	}
	return records, nil
}
...
func (d userDao) RetrieveRecord(ctx *gin.Context, options ...func(db *gorm.DB) *gorm.DB) (User, error) {
	...
	if result.Error != nil {
		return record, errors.Wrapf(base.ErrorDBSelect, "retrieve record fail, err: %s", result.Error.Error())
	}
	return record, nil
}
